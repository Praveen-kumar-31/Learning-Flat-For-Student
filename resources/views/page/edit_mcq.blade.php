<!-- resources/views/edit_mcq.blade.php -->
<!DOCTYPE html>
<html lang="en">
    <head>
        <meta charset="utf-8" />
                ...
                <div class="form-group">
                    ...
                    <select id="course_id" class="form-control" name="course_id">
                        ...
                        @foreach($courses as $course)
                            <option value="{{ $course->id }}" {{ $mcq->course_id == $course->id ? 'selected' : '' }}>{{ $course->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <div class="form-group">
                    ...
                    <select id="subtopic_id" class="form-control" name="subtopic_id">
                        ...
                        @foreach($subtopics as $subtopic)
                            <option value="{{ $subtopic->id }}" {{ $mcq->subtopic_id == $subtopic->id ? 'selected' : '' }}>{{ $subtopic->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div class="row mt-3 mx-2">
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <textarea id="question" class="form-control" name="question" rows="4">{{ $mcq->question }}</textarea>
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group">
                    <label for="answer">Answer</label>
                    <textarea id="answer" class="form-control" name="answer" rows="4">{{ $mcq->answer }}</textarea>
                </div>
            </div>
        </div>
        <div class="row mt-3 mx-2">
            <div class="col-md-4">
                <div class="form-group">
                    ...
                    <select id="type" class="form-control" name="type">
                        <option value="1" {{ $mcq->type == 1 ? 'selected' : '' }}>1</option>
                        <option value="2" {{ $mcq->type == 2 ? 'selected' : '' }}>2</option>
                        <option value="3" {{ $mcq->type == 3 ? 'selected' : '' }}>3</option>
                    </select>
                </div>
            </div>
        </div>
        <br>
        <button type="submit" class="btn btn-primary mt-3 mx-5 mb-5">Update</button>
    </form>
</div>
</main>
